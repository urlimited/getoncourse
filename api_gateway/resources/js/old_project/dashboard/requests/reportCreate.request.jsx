...
export const apiCreateReport = (data) => dispatch => {
    dispatch(core_events.eventInitRequest());

    return fetch(constants.API_CREATE_REPORT, (new DefaultRequest()).setParams({
        method: "post",
        headers: {
        ...
        .map(key => {
            if (key === "answers")
                return data["answers"].filter(a => a.answer !== CRITERIA_ANSWER_UNDEFINED)
                    .map(k => "answers=" + JSON.stringify(k)).reduce((accum, next) => accum + "&" + next);
